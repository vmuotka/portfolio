  ...
  return (
    <nav ref={wrapperRef} className='fixed top-0 left-0 z-50 w-full bg-primary-400 flex flex-wrap justify-center items-center select-none py-4'>
      <div className='block sm:hidden'>
        <button onClick={() => setShowNav(!showNav)} className='text-white hover:text-primary-100 border rounded py-1.5 px-2.5 mr-4 font-lg'>
      ...
      <div>
        <a className='font-mono sm:pr-16 py-4 font-semibold text-3xl text-white hover:text-primary-100 align-middle' href='#home'>Vili Muotka</a>
      </div>
      <div className={`${showNav && 'max-h-48'} max-h-0 sm:max-h-full overflow-hidden transition-all duration-300 ease-in-out  sm:block w-full sm:w-auto`}>
        {
          navItems.map(item =>
            <a
